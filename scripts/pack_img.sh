...
${LONGAN_CONFIG_DIR}/${PACK_BOARD#*-}/boot-resource/boot-resource:image/
${LONGAN_CONFIG_DIR}/${PACK_BOARD#*-}/boot-resource/boot-resource.ini:image/
${LONGAN_CONFIG_DIR}/${PACK_BOARD#*-}/configs/*.bmp:image/boot-resource/
)

boot_resource1_list=(
${TINA_CONFIG_DIR}/generic/boot-resource1/boot-resource1:image/
${TINA_CONFIG_DIR}/generic/boot-resource1/boot-resource1.ini:image/
${TINA_CONFIG_DIR}/${PACK_BOARD_PLATFORM}-common/boot-resource1:image/
${TINA_CONFIG_DIR}/${PACK_BOARD_PLATFORM}-common/boot-resource1.ini:image/

${LONGAN_CONFIG_DIR}/default/boot-resource1:image/
${LONGAN_CONFIG_DIR}/default/boot-resource1.ini:image/
${LONGAN_CONFIG_DIR}/${PACK_BOARD#*-}/boot-resource1/boot-resource1:image/
${LONGAN_CONFIG_DIR}/${PACK_BOARD#*-}/boot-resource1/boot-resource1.ini:image/
)
...
function do_prepare()
{
	...
	printf "copying boot resource1\n"
	for file in ${boot_resource1_list[@]} ; do
		cp -rf `echo $file | awk -F: '{print $1}'` \
			${ROOT_DIR}/`echo $file | awk -F: '{print $2}'` 2>/dev/null
	done
	...
	[ -e ${ROOT_DIR}/image/boot-resource1.ini ] && sed -i 's/\\boot-resource1/\/boot-resource1/g' ${ROOT_DIR}/image/boot-resource1.ini
...
function do_common()
{
	...
	[ -e boot-resource1.ini ] && fsbuild	     boot-resource1.ini split_xxxx.fex > /dev/null
