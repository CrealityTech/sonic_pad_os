...
update()
{
    PLAT="$(sed 's/ /\n/g' <<< "${table[@]}" | awk -F: "/${up_platform}/{print \$1}")"
    [ -z "\${PLAT}" ] && PLAT=${target}
    cur=\$(dirname \$0)
...
mk_patch()
{
    ...
    make -C ${ROOT}/${uroot} M=drivers/mmc clean &>/dev/null
    echo -e "\033[32mOK\033[0m"
    echo
    ...
    cp -r ${ROOT}/device/config/chips/${target}/bin/{boot0_sdcard_sun*.bin,u-boot-sun*.bin} \
