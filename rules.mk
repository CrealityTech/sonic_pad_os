...
ifeq ($(CONFIG_UPDATE_TOOLCHAIN), )
  ...
  ifeq ($(TARGET_CPU_VARIANT),arm926ej-s)
    ...
    ifeq ($(ARCH), riscv)
      CONFIG_TOOLCHAIN_ROOT=$(TOPDIR)/prebuilt/gcc/linux-x86/$(ARCH)/toolchain-thead-$(TOOLCHAIN_LIBC)/riscv64-glibc-gcc-thead_20200702
    else
      ifeq ($(CONFIG_COMPLILE_KERNEL64_USER32),y)
  ...
  ifneq ($(ARCH), riscv)
    ...
    CONFIG_LIBPTHREAD_ROOT_DIR="$(CONFIG_TOOLCHAIN_ROOT)/sysroot/"
    CONFIG_LIBRT_ROOT_DIR="$(CONFIG_TOOLCHAIN_ROOT)/sysroot/"
    CONFIG_LIBSSP_ROOT_DIR="$(CONFIG_TOOLCHAIN_ROOT)/sysroot/"
    CONFIG_LIBSTDCPP_ROOT_DIR="$(CONFIG_TOOLCHAIN_ROOT)/riscv64-unknown-linux-gnu/"
  endif
