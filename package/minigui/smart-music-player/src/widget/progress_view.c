...
static LRESULT ProgressViewProc(HWND hwnd, UINT message, WPARAM wParam,
        LPARAM lParam) {
    ...
    switch (message) {
    case MSG_CREATE:
        SetTimer(hwnd, ID_TIMER_PROGRESS_UPDATE, 10);
        break;
    case MSG_TIMER: {
        RECT rect;
        GetClientRect(hwnd, &rect);
        /*progressData->curPiexlProgress =
         (progressData->curPiexlProgress <= rect.right) ?
         ++progressData->curPiexlProgress : 0;*/
        GetPlayTimePercent(&progressData->curProgress);
        progressData->curPiexlProgress = rect.right * progressData->curProgress;
    ...
    case MSG_PAINT: {
        ...
        FillBox(hdc, 0, 0, progressData->curPiexlProgress, rect.bottom);

        /* Draw the right progress bar */
        SetBrushColor(hdc, RGB2Pixel(HDC_SCREEN, 51, 51, 51));
        FillBox(hdc, progressData->curPiexlProgress, 0,
                rect.right - progressData->curPiexlProgress, rect.bottom);

        /* Draw a smooth angle on the left side of the progress bar */
        if (progressData->curPiexlProgress != 0) {
        ...
        if (progressData->curPiexlProgress != rect.right) {
            SetBrushColor(hdc, RGB2Pixel(HDC_SCREEN, 46, 46, 46));
        } else {
            SetBrushColor(hdc, RGB2Pixel(HDC_SCREEN, 230, 126, 4));
        }
    ...
    case MSG_DESTROY: {
        if (IsTimerInstalled(hwnd, ID_TIMER_PROGRESS_UPDATE))
