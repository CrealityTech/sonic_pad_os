...
static void my_notify_proc(HWND hwnd, LINT id, int nc, DWORD add_data) {
    ...
    sm_debug("control button clike hwnd=%p id=%ld\n", hwnd, id);

    if (nc == STN_CLICKED) {
        /* GetDlgCtrlID(hwnd) directly passes the ID of each button */
        NotifyParentEx(GetParent(hwnd), GetDlgCtrlID(hwnd), STN_CLICKED,
                add_data);
    }
}

static LRESULT ControlButtonViewProc(HWND hwnd, UINT message, WPARAM wParam,
        LPARAM lParam) {
 ...
 */
HWND ControlButotnViewInit(HWND hosting, Uint32 flag, NOTIFPROC notif_proc) {
    smRect rect;
    ...
    if (flag == 0) {
        ...
        SetNotificationCallback(controlButtonHwnd, notif_proc);
    } else if (flag == 1) {
        ControlButotnData *controlButotnData = NULL;
