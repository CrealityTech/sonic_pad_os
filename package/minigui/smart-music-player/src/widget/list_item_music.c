...
static LRESULT ListItemArrowProc(HWND hwnd, UINT message, WPARAM wParam,
        LPARAM lParam) {
    ...
    case MSG_MUSIC_PLAY_ANIMATION_START:
        listItemMusicData->playStatus = 1;
        SetTimer(hwnd, ID_TIMER_MUSIC_PLAYING_ANIMATION, 10);
        break;
    case MSG_MUSIC_PLAY_ANIMATION_STOP:
        listItemMusicData->playStatus = 0;
        if (IsTimerInstalled(hwnd, ID_TIMER_MUSIC_PLAYING_ANIMATION)) {
            KillTimer(hwnd, ID_TIMER_MUSIC_PLAYING_ANIMATION);
        }
        UpdateWindow(hwnd, FALSE);
        break;
