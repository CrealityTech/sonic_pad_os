...
#else
#define UPDATE_INTERVALS   10   /* 20ms slide once  */
#endif

static void my_notify_proc(HWND hwnd, LINT id, int nc, DWORD add_data) {

    sm_debug("headbar back button clike hwnd=%p id=%ld\n", hwnd, id);
...
static LRESULT HeadBarViewProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case MSG_CREATE: {
        ...
        musicPlayingIndex = 0;
        if (headbarData->flag) {
            SetTimer(hwnd, ID_TIMER_MUSIC_PLAYING_ANIMATION, UPDATE_INTERVALS);
        }
        break;
    }
    case MSG_TIMER:
