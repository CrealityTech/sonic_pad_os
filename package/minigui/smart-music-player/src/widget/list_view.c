...
#define SLIDING_DOWN_UP_TIME 200    /* DOWN and UP exceeds 200 milliseconds */
#else
#define SLIDING_STEP_VALUE  4   /* Decrease 8 each time */
#define SLIDING_INTERVALS   4   /* 40ms slide once  */
#define SLIDING_MAX_NUM     50  /* Slide up to 50 times at a time */
#define SLIDING_DOWN_UP_TIME 200    /* DOWN and UP exceeds 300 milliseconds */
#endif
...
static LRESULT MyListViewProc(HWND hwnd, UINT message, WPARAM wParam,
        LPARAM lParam) {
    ...
    case MSG_MOUSE_FLING: {
        ...
        sm_debug("list view fling slidingDistance=%d velocity=%d lentime=%d\n",
                myListData->slidingDistance, velocity, lentime);

        /* The time between DOWN and UP exceeds 200 milliseconds and does not slid */
        if (velocity > 10 && lentime < SLIDING_DOWN_UP_TIME) {
            slidingNum = 0;
