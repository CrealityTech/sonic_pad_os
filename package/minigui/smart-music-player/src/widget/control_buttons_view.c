...
static void my_notify_proc(HWND hwnd, LINT id, int nc, DWORD add_data) {
    ...
    sm_debug("controls button clike hwnd=%p id=%ld\n", hwnd, id);
...
static LRESULT ControlButtonsViewProc(HWND hwnd, UINT message, WPARAM wParam,
        LPARAM lParam) {
...
HWND ControlButotnsViewInit(HWND hosting, NOTIFPROC notif_proc) {
    smRect rect;
