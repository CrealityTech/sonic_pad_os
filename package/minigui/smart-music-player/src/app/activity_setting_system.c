...
static void my_notify_proc(HWND hwnd, LINT id, int nc, DWORD add_data) {
    ...
    sm_debug("setting button clike hwnd=%p id=%ld\n", hwnd, id);
    ...
    if (nc == STN_CLICKED) {
        ...
        case 1:
            ...
            break;
        default:
            sm_error("Do not open any windows id is %ld\n", id);
            break;
        }
    }
}

static LRESULT ActivitySettingSystemProc(HWND hWnd, UINT nMessage, WPARAM wParam,
        LPARAM lParam) {
