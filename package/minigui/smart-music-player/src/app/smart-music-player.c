...
static LRESULT MainWindowProc(HWND hWnd, UINT nMessage, WPARAM wParam,
        LPARAM lParam) {
    ...
    case MSG_PLAYBACK_COMPLETE:
        /* ActivityMusicList will also play the next song  */
        if (isActivityMusicListOpen)
            break;
...
void MusicListInit(void) {
    ...
    if (!dp) {
        sm_debug("open directory %s error\n", MUSIC_LIST_DIR);
        return;
    }
