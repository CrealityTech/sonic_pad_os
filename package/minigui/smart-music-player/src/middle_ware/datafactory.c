...
ControlButotnsData *ControlButotnsDataInit(
    ...
    /* Volume button can be clicked continuously */
    controlButotnsData->buttonData[0].doubleClickDisable = TRUE;
    controlButotnsData->buttonData[2].doubleClickDisable = TRUE;
...
ListItemMusicData *ListItemMusicDataInit(ListItemMusicData *listItemMusicData,
    ...
    /* Intercept string.mp3 */
    char extr[FILE_NAME_MAXT_LEN] = { '0' };
    list_node_t *tmp_node = list_get_current_node(media_list);
    strncpy(extr, tmp_node->name, strlen(tmp_node->name) - 4);

    strcpy(listItemMusicData->musicText, extr);
    listItemMusicData->musicIndex = list_get_cur_index(media_list);
...
MyListData *MyListDataInit(MyListData *myListData, Uint32 index) {
    ...
    if (index == 0) {
        ...
        if (!list_get_total_num(media_list))
            myListData->listSize = 0;
        else
            myListData->listSize = MUSIC_LIST_VIEW_SIZE;

        int cur_index = list_get_cur_index(media_list);
