...
{
	int opt;
	const char *opts = "c:hslnrtdoie";
	char scan_results[SCAN_MAX] = {0};
	char list_net_results[LIST_NETWORK_MAX] = {0};
	int debug_level = 0;
	...
	struct option longopts[] = {
		{"reconnect",required_argument,NULL,'n'},
	...
	while ((opt = getopt_long(argc, argv, opts, longopts, NULL)) != -1){
		switch(opt){
			...
			case 'n'/* --commnad */:
				if(argc != 3){
					printf("  -n, --reconnect\treconnect AP,-n <netid>\n");
					return EXIT_SUCCESS;
				}
				printf("==============================================\n");
				printf("reconnecting netid:%s\n",argv[optind]);
				aw_wifid_reconnect_ap_with_netid(argv[optind],&event);
...
usage:
				...
				printf("  -i, --info\t\tget wifi info\n");
				printf("  -e, --disconnect\tdisconnect AP\n");
			...
			case 'i'/*--info*/:
				ret = aw_wifid_get_connect_info(&connect_info);
				if(ret >= 0) {
					wmg_printf(MSG_INFO,"connected ip:%s\n",connect_info.ip_address);
				}
				return EXIT_SUCCESS;
