...
static void wifid_reconnect_with_netid(const struct da_requst *r, const aw_wifi_interface_t *p_wifi_interface,int fd)
{
	enum cmd_status status = DA_CMD_SUCCESS;
	enum cn_event event = DA_UNKNOWN;
	enum wmgEvent wmg_event;

	wmg_printf(MSG_DEBUG,"ctl command: reconnecet start -+- \n");

	if(NULL == p_wifi_interface)
		goto code_failed;

	p_wifi_interface->connect_ap_with_netid(r->ssid,fd);
...
code_failed:
	...
	send(fd, &status, sizeof(status), MSG_NOSIGNAL);
	wmg_printf(MSG_DEBUG,"ctl command: reconnecet end -+- \n");
}

static void wifid_disconnect(const struct da_requst *r, const aw_wifi_interface_t *p_wifi_interface,int fd)
{
	enum cmd_status status = DA_CMD_SUCCESS;

	wmg_printf(MSG_DEBUG,"ctl command: disconnecet start -+- \n");

	if(NULL == p_wifi_interface)
		goto code_failed;

	if( p_wifi_interface->disconnect_ap(fd) >= 0)
		goto end;
...
code_failed:
	...
	send(fd, &status, sizeof(status), MSG_NOSIGNAL);
	wmg_printf(MSG_DEBUG,"ctl command: disconnecet end -+- \n");
}
...
static void wifid_connection_info(const struct da_requst *r, const aw_wifi_interface_t *p_wifi_interface,int fd)
{
	...
	wmg_printf(MSG_DEBUG,"ctl cmd: connection info start -+- \n");

	if(NULL == p_wifi_interface)
		goto code_failed;

	if(p_wifi_interface->get_connection_info(&info) < 0)
		goto code_failed;

	if(msg_pipe_write((char*)&info,sizeof(info)) >= 0)
		goto end;
...
code_failed:
	...
	send(fd, &status, sizeof(status), MSG_NOSIGNAL);
	wmg_printf(MSG_DEBUG,"ctl cmd: connection info end -+- \n");
}
...
void ctl_loop(const aw_wifi_interface_t *p_wifi_interface)
{
	static void (*commands[__DA_COMMAND_MAX])(const struct da_requst *,const aw_wifi_interface_t * ,int) = {
		...
		[DA_COMMAND_RECONNECT] = wifid_reconnect_with_netid,
		...
		[DA_COMMAND_DISCONNECT] = wifid_disconnect,
		[DA_COMMAND_CONNECT_INFO] = wifid_connection_info,
	};
