...
{
	struct da_requst req = {
		.command = DA_COMMAND_CONNECT_INFO,
		.ssid = {0},
		.pwd = {0},
	};
	struct client cli = {
	...
	if(cli.enable_pipe){
		ret = read_command_message(cli.pipe_fd,(char*)info,sizeof(connection_status));
	}
	handle_command_free(&cli);
	return ret ;
}
...
int aw_wifid_reconnect_ap_with_netid(const char *ssid, enum cn_event *ptrEvent)
{

	struct da_requst req = {
		.command = DA_COMMAND_RECONNECT,
		.ssid = {0},
		.pwd = {0},
	};
	struct client cli = {
