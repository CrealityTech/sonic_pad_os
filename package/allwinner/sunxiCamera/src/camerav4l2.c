...
int streamon(camera_hal *camera)
{
...
#ifdef __USE_VIN_ISP__
	{
		...
		/* setting ISP */
		if (isp &&
			(isp->sensor_type == V4L2_SENSOR_TYPE_RAW) &&
			(isOutputRawData(camera->format) == 0)) {
			isp->isp_id = -1;
...
int dqbuf(camera_hal *camera)
{
	...
	camera->buffers[buf.index].image_timestamp = ((long long)buf.timestamp.tv_sec)*1000000 + buf.timestamp.tv_usec;
