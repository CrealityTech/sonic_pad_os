...
int ion_alloc_get_fd_by_viraddr_cpu(void* pbuf)
{
    int flag = 0;
    unsigned long addr_vir = (unsigned long)pbuf;
    ...
    if (0 == flag)
    {
        loge("ion_alloc_phy2vir failed, do not find addr_vir address: 0x%lx \n", addr_vir);
    }
...
void* ion_alloc_get_viraddr_by_fd_cpu(int nShareFd)
{
    int flag = 0;
    unsigned long addr_vir = 0;
    buffer_node * tmp = NULL;
...
struct ScMemOpsS _ionMemOpsS =
{
    ...
    .cpu_get_fd_by_viraddr = ion_alloc_get_fd_by_viraddr_cpu,
    .cpu_get_viraddr_by_fd = ion_alloc_get_viraddr_by_fd_cpu
};
