...
struct ScMemOpsS
{
    ...
    int (*cpu_get_fd_by_viraddr)(void* /*virtual address*/);

    void *(*cpu_get_viraddr_by_fd)(int /*fd*/);
};
...
static inline int CdcMemPallocGetFdByVirtualAddr(struct ScMemOpsS *memops,
    void *virt)
{
    return memops->cpu_get_fd_by_viraddr(virt);
}

static inline void *CdcMemPallocGetVirtualAddrByFd(struct ScMemOpsS *memops,
    int share_fd)
{
    return memops->cpu_get_viraddr_by_fd(share_fd);
}
