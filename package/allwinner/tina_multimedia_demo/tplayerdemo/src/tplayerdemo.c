...
int main(int argc, char** argv)
{
    ...
    printf("******************************************************************************************\n");
    if(argc == 3){//test play 2 video at the same time
        printf("argc = %d,argv[0] = %s,argv[1] = %s,argv[2] = %s\n",argc,argv[0],argv[1],argv[2]);
        char fileType[VIDEO_TYPE_NUM][VIDEO_TYPE_LEN] = {".avi",".mkv",".flv",".ts",".mp4",".ts",".webm",".asf",".mpg",".mov",".vob"};
        char* lastStrPos;
        ...
        for(int i = 0;i < argc-1;i++){
          if((lastStrPos = strrchr(argv[i+1],'.')) != NULL){
              ...
              //* create a player.
              memset(&gTwoDemoPlayer[i], 0, sizeof(DemoPlayerContext));
              gTwoDemoPlayer[i].mTPlayer= TPlayerCreate(CEDARX_PLAYER);
              if(gTwoDemoPlayer[i].mTPlayer == NULL)
              {
                  printf("can not create tplayer, quit.\n");
                  exit(-1);
              }else{
                  printf("create player[%d]:%p\n",i,gTwoDemoPlayer[i].mTPlayer);
              }
              //* set callback to player.
              TPlayerSetNotifyCallback(gTwoDemoPlayer[i].mTPlayer,CallbackForTPlayer, (void*)&gTwoDemoPlayer[i]);
              if(TPlayerSetDataSource(gTwoDemoPlayer[i].mTPlayer,(const char*)argv[i+1],NULL)!= 0)
              {
              ...
              TPlayerPrepare(gTwoDemoPlayer[i].mTPlayer);
              gTwoDemoPlayer[i].mComplete = 0;
              gTwoDemoPlayer[i].mError = 0;
              if(i == 0)
                TPlayerSetDisplayRect(gTwoDemoPlayer[i].mTPlayer, 0, 0, 400, 400);
              else
                TPlayerSetDisplayRect(gTwoDemoPlayer[i].mTPlayer, 400, 400, 400, 400);
               if(TPlayerStart(gTwoDemoPlayer[i].mTPlayer) != 0)
              {
        ...
        while(gTwoDemoPlayer[0].mComplete==0 || gTwoDemoPlayer[1].mComplete==0){
              if(gTwoDemoPlayer[0].mError == 1 || gTwoDemoPlayer[1].mError == 1){
                printf("has error,do not play the video\n");
                break;
              }
        ...
        for(i = 0;i < argc-1;i++){
            if(gTwoDemoPlayer[i].mTPlayer != NULL)
            {
                TPlayerDestroy(gTwoDemoPlayer[i].mTPlayer);
                gTwoDemoPlayer[i].mTPlayer = NULL;
                printf("TPlayerDestroy() successfully\n");
            }
            printf("destroy tplayer \n");
        }
