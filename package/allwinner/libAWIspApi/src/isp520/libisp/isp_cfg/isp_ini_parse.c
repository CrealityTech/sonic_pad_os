...
#include "SENSOR_H/gc8034_mipi_for_r311.h"
...
struct isp_cfg_array cfg_arr[] = {
	...
	{"gc8034_mipi", "gc5035_mipi_isp_cfg", 3264, 2448, 25, 0, 0, &gc8034_mipi_isp_cfg},
};
			...
			int w, int h, int fps, int wdr, int ir, int sync_mode, int isp_id)
{
	...
	if (i == array_size(cfg_arr)) {
		...
		} else if (!strncmp(sensor_name, "gc8034_mipi", 11)) {
			ISP_PRINT("use gc8034_mipi isp config.\n");
			cfg = &gc8034_mipi_isp_cfg;
		} else {
