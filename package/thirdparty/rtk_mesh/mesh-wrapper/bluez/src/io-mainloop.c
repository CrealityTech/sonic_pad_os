...
static struct io *io_ref(struct io *io)
{
	if (!io)
	{
		DFU_PRINT_ERROR0(" io is null");
		if(errno != 0){
			DFU_PRINT_ERROR2("errno %d, %s",errno,strerror(errno));
		}
		return NULL;
	}
...
static void io_callback(int fd, uint32_t events, void *user_data)
{
	...
	if ((events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR))) {
		...
		if (!io->disconnect_callback(io, io->disconnect_data)) {
			...
			mainloop_modify_fd2(io->fd, io->events);
		}
	}
...
struct io *io_new2(int fd)
{
	...
	if (mainloop_add_fd2(io->fd, io->events, io_callback,
						io, io_cleanup) < 0) {

		DFU_PRINT_ERROR0("mainloop_add_fd2 failed");
		if(errno != 0){
			DFU_PRINT_ERROR2("errno %d, %s",errno,strerror(errno));
		}
		free(io);
...
bool io_set_close_on_destroy2(struct io *io, bool do_close)
{
	if (!io)
		return false;
...
bool io_set_read_handler2(struct io *io, io_callback_func_t callback,
				void *user_data, io_destroy_func_t destroy)
{
	...
	if (!io || io->fd < 0)
	{
		DFU_PRINT_ERROR2(" io is NULL? %s, io->fd<0? %s",io==NULL?"YES":"NO",io->fd<0?"YES":"NO");
		if(errno != 0){
			DFU_PRINT_ERROR2("errno %d, %s",errno,strerror(errno));
		}
		return false;
	...
	if (mainloop_modify_fd2(io->fd, events) < 0)
	{
		DFU_PRINT_ERROR0(" mainloop_modify_fd2(io->fd, events) < 0");
		if(errno != 0){
			DFU_PRINT_ERROR2("errno %d, %s",errno,strerror(errno));
		}
		return false;		
...
bool io_set_write_handler2(struct io *io, io_callback_func_t callback,
				void *user_data, io_destroy_func_t destroy)
{
	uint32_t events;
	...
	//DFU_PRINT_TRACE1("modify %lu", pthread_self());
	if (mainloop_modify_fd2(io->fd, events) < 0)
		return false;
...
ssize_t io_send2(struct io *io, const struct iovec *iov, int iovcnt)
{
	ssize_t ret;
