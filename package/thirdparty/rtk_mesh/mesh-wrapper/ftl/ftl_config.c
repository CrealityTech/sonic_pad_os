...
config_t *config_new_empty(void) {
  ...
  if (!config->sections) {
    OS_PRINT_ERROR1("%s unable to allocate list for sections.", __func__);
    goto error;
  }
...
config_t *config_new(const char *filename) {
  ...
  if (!fp) {
    OS_PRINT_ERROR3("%s unable to open file '%s': %s", __func__, filename, strerror(errno));
    config_free(config);
    return NULL;
  }
...
bool config_save(const config_t *config, const char *filename) {
  ...
  if (!directoryname) {
    OS_PRINT_ERROR3("%s error extracting directory from '%s': %s", __func__, filename, strerror(errno));
    goto error;
  }

  dir_fd = open(directoryname, O_RDONLY);
  if (dir_fd < 0) {
    OS_PRINT_ERROR3("%s unable to open dir '%s': %s", __func__, directoryname, strerror(errno));
    goto error;
  }

  fp = fopen(temp_filename, "wt");
  if (!fp) {
    OS_PRINT_ERROR3("%s unable to write file '%s': %s", __func__, temp_filename, strerror(errno));
    goto error;
  }
  ...
  for (const list_node_t *node = list_begin(config->sections); node != list_end(config->sections); node = list_next(node)) {
    ...
    if (fprintf(fp, "[%s]\n", section->name) < 0) {
      OS_PRINT_ERROR3("%s unable to write to file '%s': %s", __func__, temp_filename, strerror(errno));
      goto error;
    }
  ...
  if (fsync(fileno(fp)) < 0) {
    OS_PRINT_WARN3("%s unable to fsync file '%s': %s", __func__, temp_filename, strerror(errno));
  }

  if (fclose(fp) == EOF) {
    OS_PRINT_ERROR3("%s unable to close file '%s': %s", __func__, temp_filename, strerror(errno));
    goto error;
  }
  fp = NULL;
  ...
  if (chmod(temp_filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP) == -1) {
    OS_PRINT_ERROR3("%s unable to change file permissions '%s': %s", __func__, filename, strerror(errno));
    goto error;
  }

  // Rename written temp file to the actual config file.
  if (rename(temp_filename, filename) == -1) {
    OS_PRINT_ERROR3("%s unable to commit file '%s': %s", __func__, filename, strerror(errno));
    goto error;
  }

  // This should ensure the directory is updated as well.
  if (fsync(dir_fd) < 0) {
    OS_PRINT_WARN3("%s unable to fsync dir '%s': %s", __func__, directoryname, strerror(errno));
  }

  if (close(dir_fd) < 0) {
    OS_PRINT_ERROR3("%s unable to close dir '%s': %s", __func__, directoryname, strerror(errno));
    goto error;
  }
...
static bool config_parse(FILE *fp, config_t *config) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (*line_ptr == '[') {
      ...
      if (line_ptr[len - 1] != ']') {
        OS_PRINT_TRACE2("%s unterminated section name on line %d.", __func__, line_num);
        return false;
      }
      ...
      if (!split) {
        OS_PRINT_TRACE2("%s no key/value separator found on line %d.", __func__, line_num);
        return false;
      }

      *split = '\0';
      config_set_string(config, section, trim(line_ptr), trim1(split + 1));
    }
  }
  return true;
}
