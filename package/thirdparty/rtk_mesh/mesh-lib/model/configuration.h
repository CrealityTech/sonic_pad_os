 ...
 * @{
 */
#define CFG_CLIENT_APP_KEY_STATUS                       0
#define CFG_CLIENT_COMPO_DATA_STATUS                    1
#define CFG_CLIENT_BEACON_STATUS                        2
#define CFG_CLIENT_DEFAULT_TTL_STATUS                   3
#define CFG_CLIENT_FRND_STATUS                          4
#define CFG_CLIENT_PROXY_STATUS                         5
#define CFG_CLIENT_KEY_REFRESH_PHASE_STATUS             6
#define CFG_CLIENT_MODEL_PUB_STATUS                     7
#define CFG_CLIENT_MODEL_SUB_STATUS                     8
#define CFG_CLIENT_NET_TRANS_STATUS                     9
#define CFG_CLIENT_RELAY_STATUS                         10
#define CFG_CLIENT_LPN_POLL_TO_STATUS                   11
#define CFG_CLIENT_HB_PUB_STATUS                        12
#define CFG_CLIENT_HB_SUB_STATUS                        13
#define CFG_CLIENT_MODEL_APP_STATUS                     14
#define CFG_CLIENT_NET_KEY_STATUS                       15
#define CFG_CLIENT_NODE_IDENTITY_STATUS                 16
#define CFG_CLIENT_NODE_RESET_STATUS                    17
#define CFG_CLIENT_SIG_MODEL_SUB_LIST					18
#define CFG_CLIENT_VENDOR_MODEL_SUB_LIST				19
#define CFG_CLIENT_NET_KEY_LIST                         20
#define CFG_CLIENT_APP_KEY_LIST                         21
#define CFG_CLIENT_SIG_MODEL_APP_LIST                 	22
#define CFG_CLIENT_VENDOR_MODEL_APP_LIST                23

typedef struct
{
    uint16_t cfg_server_addr;
    mesh_msg_stat_t stat;
    uint16_t element_addr;
    uint32_t model_id; //!< 4 bytes
    uint16_t appKeyNum;
    uint16_t *pAppkeyIndexes; //!< variable length
} cfg_client_list_model_app_t;
...
typedef struct
{
	...
	uint16_t appkeyNum;
    uint16_t *pAppkeyIndexes; //!< variable length
} cfg_client_list_app_key_t;

typedef struct
{
    uint16_t cfg_server_addr;
	uint16_t netkeyNum;
    uint16_t *pNetkeyIndexes; //!< variable length
} cfg_client_list_net_key_t;
...
typedef struct
{
    ...
    uint16_t element_addr;
    uint32_t model_id; //!< 4 bytes
    uint16_t listElementNum;
    uint16_t *pAddrArray; //!< variable length
} cfg_client_list_model_sub_t;

typedef struct
{
    uint16_t cfg_server_addr;
    uint8_t state;
} cfg_client_status_beacon_t;

typedef struct
{
    uint16_t cfg_server_addr;
    uint8_t page;
	uint16_t compoDataLen;
    uint8_t *pCompoData; //!< variable length
} cfg_client_status_compo_data_t;

typedef struct
{
    uint16_t cfg_server_addr;
    uint8_t ttl;
} cfg_client_status_default_ttl_t;

typedef struct
{
    uint16_t cfg_server_addr;
    uint8_t state;
} cfg_client_status_proxy_t;
...
typedef struct
{
    ...
    uint8_t count : 3;
    uint8_t steps : 5;
} cfg_client_status_net_transmit_t;
...
typedef struct
{
    ...
    uint16_t net_key_index;
    uint8_t identity;
} cfg_client_status_node_identity_t;
...
typedef struct
{
    ...
    uint16_t app_key_index;
    uint32_t model_id; //!< 2 bytes or 4 bytes
} cfg_client_status_model_app_t;

typedef struct
{
    uint16_t cfg_server_addr;
} cfg_client_status_node_reset_t;

typedef struct
{
    uint16_t cfg_server_addr;
    uint8_t state;
} cfg_client_status_frnd_t;
...
typedef struct
{
    ...
    uint16_t lpn_addr;
    uint32_t poll_to;
} cfg_client_status_lpn_poll_timeout_t;
...
typedef struct
{
    ...
    uint16_t net_key_index;
    uint8_t state;
} cfg_client_status_key_refresh_phase_t;
