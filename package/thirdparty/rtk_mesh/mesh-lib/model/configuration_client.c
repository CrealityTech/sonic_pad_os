    ...
    case MESH_MSG_CFG_BEACON_STAT:
        {
			...
			cfg_client_status_beacon_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.state = pmsg->state;
            if (NULL != pmesh_msg->pmodel_info->model_data_cb)
            {
                pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_BEACON_STATUS,
                                                      &status_data);
            }
        }
        break;
    case MESH_MSG_CFG_COMPO_DATA_STAT:
        {
			...
			cfg_client_status_compo_data_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.page = pmsg->page;
			status_data.compoDataLen = pmesh_msg->msg_len-2;
			status_data.pCompoData = plt_malloc(status_data.compoDataLen*sizeof(uint8_t),0);
			if(status_data.pCompoData != NULL)
			{
				memcpy(status_data.pCompoData, pmsg->data, status_data.compoDataLen);

				if (NULL != pmesh_msg->pmodel_info->model_data_cb)
				{
					pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_COMPO_DATA_STATUS,
														  &status_data);
				}
				plt_free(status_data.pCompoData,0);
			}
			else
			{
				printe("MESH_MSG_CFG_COMPO_DATA_STAT no memory to callback");
			}
...
compo_data_end:
    ...
    case MESH_MSG_CFG_DEFAULT_TTL_STAT:
        {
			...
			cfg_client_status_default_ttl_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.ttl = pmsg->ttl;

            if (NULL != pmesh_msg->pmodel_info->model_data_cb)
            {
                pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_DEFAULT_TTL_STATUS,
                                                      &status_data);
            }
        }
        break;
    case MESH_MSG_CFG_PROXY_STAT:
        {
			...
			cfg_client_status_proxy_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.state = pmsg->state;

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_PROXY_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_RELAY_STAT:
        {
			...
			cfg_client_status_relay_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.state = pmsg->state;
			status_data.count = pmsg->count;
			status_data.steps = pmsg->steps;

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_RELAY_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_NET_TRANS_STAT:
        {
			...
			cfg_client_status_net_transmit_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.count = pmsg->count;
			status_data.steps = pmsg->steps;
			...
			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_NET_TRANS_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_MODEL_PUB_STAT:
        {
			...
			cfg_client_status_model_pub_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.element_addr = pmsg->element_addr;
			status_data.pub_addr = pmsg->pub_addr;
			status_data.pub_key_info = pmsg->pub_key_info;
			status_data.pub_ttl = pmsg->pub_ttl;
			status_data.pub_period = pmsg->pub_period;
			if(pmesh_msg->msg_len == 12) //2Bytes Model id
			{
				status_data.model_id = pmsg->model_id & 0xffff;
			}
			else // 4Bytes Model id
			{
				status_data.model_id = pmsg->model_id;
			}

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_MODEL_PUB_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_MODEL_SUB_STAT:
        {
			...
			cfg_client_status_model_sub_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.element_addr = pmsg->element_addr;
			status_data.addr = pmsg->addr;
			if(pmesh_msg->msg_len == 7) //2Bytes Model id
			{
				status_data.model_id = pmsg->model_id & 0xffff;
			}
			else //4Bytes Model id
			...
			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_MODEL_SUB_STATUS,
													  &status_data);
			}
        }
        break;
    ...
    case MESH_MSG_CFG_VENDOR_MODEL_SUB_LIST:
        {
            ...
            if (pmsg->stat == MESH_MSG_STAT_SUCCESS)
            {
                ...
                pmsg_data= pbuffer + len;
                for (uint16_t loop = 0; loop < pmesh_msg->msg_len - len; loop += 2)
                {
                    data_uart_debug(" 0x%04x", LE_EXTRN2WORD(pmsg_data + loop));
                }
                data_uart_debug("\r\n");
            }
            else
			...
			uint16_t len = MEMBER_OFFSET(cfg_model_sub_list_t,model_id)
							+ (pmesh_msg->access_opcode == MESH_MSG_CFG_SIG_MODEL_SUB_LIST ? 2 : 4);
			cfg_client_list_model_sub_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.element_addr = pmsg->element_addr;
			status_data.model_id = pmesh_msg->access_opcode == MESH_MSG_CFG_SIG_MODEL_SUB_LIST ?
                                    MESH_MODEL_TRANSFORM(pmsg->model_id) : pmsg->model_id;
			status_data.listElementNum = (pmesh_msg->msg_len - len)/2;
			if(status_data.listElementNum == 0 )
			{
				status_data.pAddrArray = NULL;
			}
			else
			{
				status_data.pAddrArray = plt_malloc(status_data.listElementNum*sizeof(uint16_t),0);
				if(status_data.pAddrArray != NULL)
				{
					memset(status_data.pAddrArray, 0, status_data.listElementNum*sizeof(uint16_t));
					pmsg_data= pbuffer + len;
					for(uint16_t index =0 ;index < status_data.listElementNum; index++)
					{
						status_data.pAddrArray[index]= LE_EXTRN2WORD(pmsg_data + (index*2));
					}
				}
				else
				{
					printe("MESH_MSG_CFG_MODEL_SUB_LIST no memory to callback");
				}
			}

			if(status_data.listElementNum == 0 || status_data.pAddrArray != NULL)
			{
				uint32_t type = pmesh_msg->access_opcode ==MESH_MSG_CFG_SIG_MODEL_SUB_LIST ?
								CFG_CLIENT_SIG_MODEL_SUB_LIST : CFG_CLIENT_VENDOR_MODEL_SUB_LIST;
				if (NULL != pmesh_msg->pmodel_info->model_data_cb)
				{
					pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, type,
														  &status_data);
				}
				if(status_data.pAddrArray != NULL)
				{
					plt_free(status_data.pAddrArray, 0);
				}
			}
        }
        break;
    case MESH_MSG_CFG_NET_KEY_STAT:
        {
            if (pmesh_msg->msg_len == sizeof(cfg_net_key_stat_t))
            {
				...
				cfg_client_status_net_key_t status_data;
				status_data.cfg_server_addr = pmesh_msg->src;
				status_data.stat = pmsg->stat;
				status_data.net_key_index = pmsg->net_key_index;

				if (NULL != pmesh_msg->pmodel_info->model_data_cb)
				{
					pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_NET_KEY_STATUS,
														  &status_data);
				}
            }
        }
    ...
    case MESH_MSG_CFG_NET_KEY_LIST:
        {
			cfg_client_list_net_key_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.netkeyNum = 0;
			status_data.pNetkeyIndexes = NULL;
            ...
            if (remainder == 0 || remainder == 2)
            {
				...
				status_data.netkeyNum = key_count;
				if(key_count != 0)
				{
					status_data.pNetkeyIndexes = plt_malloc(key_count*sizeof(uint16_t),0);
					if(status_data.pNetkeyIndexes == NULL)
					{
						printe("CFG_CLIENT_NET_KEY_LIST no memory to callback");
					}
					else
					{
						memset(status_data.pNetkeyIndexes, 0, key_count*sizeof(uint16_t));
					}
				}
                ...
                for (uint16_t loop = 0; loop < key_count; loop++)
                {
					...
					if(status_data.pNetkeyIndexes != NULL)
					{
						status_data.pNetkeyIndexes[loop] = key_index;
					}
                    data_uart_debug(" 0x%03x", key_index);
                }
				if(status_data.netkeyNum == 0 || status_data.pNetkeyIndexes != NULL)
				{
					if (NULL != pmesh_msg->pmodel_info->model_data_cb)
					{
						pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_NET_KEY_LIST,
															  &status_data);
					}
				}
				if(status_data.pNetkeyIndexes != NULL)
				{
					plt_free(status_data.pNetkeyIndexes, 0);
				}
                data_uart_debug("\r\n");
            }
            else
            {
				...
				printe("CFG_CLIENT_NET_KEY_LIST, fail, len = %d!",len);
            }
        }
        break;
    case MESH_MSG_CFG_APP_KEY_STAT:
        {
            if (pmesh_msg->msg_len == sizeof(cfg_app_key_stat_t))
            {
				...
				cfg_client_status_app_key_t status_data;
				status_data.cfg_server_addr = pmesh_msg->src;
				status_data.stat = pmsg->stat;
				memcpy(status_data.key_index,pmsg->key_index,3);

				if (NULL != pmesh_msg->pmodel_info->model_data_cb)
				{
					pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_APP_KEY_STATUS,
														  &status_data);
				}
            }
        }
    ...
    case MESH_MSG_CFG_APP_KEY_LIST:
        {
			...
			cfg_client_list_app_key_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.net_key_index = pmsg->net_key_index;
			status_data.appkeyNum = 0;
			status_data.pAppkeyIndexes = NULL;
            ...
            if (pmsg->stat == MESH_MSG_STAT_SUCCESS && (remainder == 0 || remainder == 2))
            {
                parse = TRUE;
                uint16_t key_count = len / 3 * 2 + (remainder != 0);
				status_data.appkeyNum = key_count;
				if(key_count != 0)
				{
					status_data.pAppkeyIndexes = plt_malloc(key_count*sizeof(uint16_t),0);
					if(status_data.pAppkeyIndexes == NULL)
					{
						printe("CFG_CLIENT_APP_KEY_LIST no memory to callback");
					}
					else
					{
						memset(status_data.pAppkeyIndexes, 0, key_count*sizeof(uint16_t));
					}
				}
                ...
                for (uint16_t loop = 0; loop < key_count; loop++)
                {
					...
					if(status_data.pAppkeyIndexes != NULL)
					{
						status_data.pAppkeyIndexes[loop] = key_index;
					}
                    data_uart_debug(" 0x%03x", key_index);
                }
				data_uart_debug("\r\n");
			...
			if(remainder == 0 || remainder == 2)
			{
				if(status_data.appkeyNum == 0 || status_data.pAppkeyIndexes != NULL)
				{
					if (NULL != pmesh_msg->pmodel_info->model_data_cb)
					{
						pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_APP_KEY_LIST,
															  &status_data);
					}
				}
				if(status_data.pAppkeyIndexes != NULL)
				{
					plt_free(status_data.pAppkeyIndexes, 0);
				}
			}
			else
			{
				printe("CFG_CLIENT_APP_KEY_LIST Fail, len = %d!\r\n", len);
			}
        }
        break;
    case MESH_MSG_CFG_NODE_IDENTITY_STAT:
        {
			...
			cfg_client_status_node_identity_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.net_key_index = pmsg->net_key_index;
			status_data.identity = pmsg->identity;

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_NODE_IDENTITY_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_MODEL_APP_STAT:
        {
			...
			cfg_client_status_model_app_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.element_addr = pmsg->element_addr;
			status_data.app_key_index = pmsg->app_key_index;
			if(pmesh_msg->msg_len == 7)
			{
				status_data.model_id = pmsg->model_id & 0xffff;
			}
			else
			...
			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_MODEL_APP_STATUS,
													  &status_data);
			}
        }
        break;
    ...
    case MESH_MSG_CFG_VENDOR_MODEL_APP_LIST:
        {
			...
			cfg_client_list_model_app_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.element_addr = pmsg->element_addr;
			status_data.model_id = 0;
			status_data.appKeyNum = 0;
			status_data.pAppkeyIndexes = NULL;
            ...
            if (pmsg->stat == MESH_MSG_STAT_SUCCESS && (remainder == 0 || remainder == 2))
            {
				...
				if(key_count != 0)
				{
					...
					if(status_data.pAppkeyIndexes == NULL)
					{
						printe("CFG_CLIENT_MODEL_APP_LIST no memory to callback");
					}
					else
					{
						memset(status_data.pAppkeyIndexes,0,key_count*sizeof(uint16_t));
			...
			if(remainder == 0 || remainder == 2)
			{
				uint32_t type = pmesh_msg->access_opcode ==MESH_MSG_CFG_SIG_MODEL_APP_LIST ?
								CFG_CLIENT_SIG_MODEL_APP_LIST : CFG_CLIENT_VENDOR_MODEL_APP_LIST;
				if(status_data.appKeyNum == 0 || status_data.pAppkeyIndexes != NULL)
				{
					if (NULL != pmesh_msg->pmodel_info->model_data_cb)
					{
			...
			{
				printe("CFG_CLIENT_MODEL_APP_LIST Fail len = %d!", len);
			}
    ...
    case MESH_MSG_CFG_NODE_RESET_STAT:
        {
			...
			cfg_client_status_node_reset_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_NODE_RESET_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_FRND_STAT:
        {
			...
			cfg_client_status_frnd_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.state = pmsg->state;

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_FRND_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_LPN_POLL_TO_STAT:
        {
			...
			cfg_client_status_lpn_poll_timeout_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.lpn_addr = pmsg->lpn_addr;
			status_data.poll_to = pmsg->poll_to[0] + (pmsg->poll_to[1] << 8) + (pmsg->poll_to[2] << 16);

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_LPN_POLL_TO_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_KEY_REFRESH_PHASE_STAT:
        {
			...
			cfg_client_status_key_refresh_phase_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.net_key_index = pmsg->net_key_index;
			status_data.state = pmsg->state;
			...
			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_KEY_REFRESH_PHASE_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_HB_PUB_STAT:
        {
			...
			cfg_client_status_hb_pub_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.dst = pmsg->dst;
			status_data.count_log = pmsg->count_log;
			status_data.period_log = pmsg->period_log;
			status_data.ttl = pmsg->ttl;
			status_data.features = pmsg->features;
			status_data.net_key_index = pmsg->net_key_index;

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_HB_PUB_STATUS,
													  &status_data);
			}
        }
        break;
    case MESH_MSG_CFG_HB_SUB_STAT:
        {
			...
			cfg_client_status_hb_sub_t status_data;
			status_data.cfg_server_addr = pmesh_msg->src;
			status_data.stat = pmsg->stat;
			status_data.src = pmsg->src;
			status_data.dst = pmsg->dst;
			status_data.period_log = pmsg->period_log;
			status_data.count_log = pmsg->count_log;
			status_data.min_hops = pmsg->min_hops;
			status_data.max_hops = pmsg->max_hops;

			if (NULL != pmesh_msg->pmodel_info->model_data_cb)
			{
				pmesh_msg->pmodel_info->model_data_cb(pmesh_msg->pmodel_info, CFG_CLIENT_HB_SUB_STATUS,
													  &status_data);
			}
        }
        break;
