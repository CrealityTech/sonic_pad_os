 ...
 *         which paramter shall be update.
 *
 * @return Operation result.
 * @retval GAP_CAUSE_SUCCESS Operation success.
 * @retval Others Operation failure.
 ...
 * \code{.c}
    void test()
    {
        uint16_t adv_min_interval = 0x80;
        uint16_t adv_max_interval = 0x80;

        le_adv_set_param(GAP_PARAM_ADV_INTERVAL_MIN, sizeof(adv_min_interval), &adv_min_interval);
        le_adv_set_param(GAP_PARAM_ADV_INTERVAL_MAX, sizeof(adv_max_interval), &adv_max_interval);
        le_adv_update_param();
    }
...
/**
  * @brief  Set the advertising tx power for the device.
...
T_GAP_CAUSE le_vendor_set_scan_para_when_a2dp_active(uint16_t scan_interval, uint16_t scan_windows);

T_GAP_CAUSE le_vendor_set_scan_high_prioirty(uint8_t enable, uint8_t highTime );
