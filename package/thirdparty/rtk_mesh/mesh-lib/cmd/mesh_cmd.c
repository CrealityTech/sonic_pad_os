...
extern uint8_t gap_get_param(T_GAP_PARAM_TYPE param, void *p_value);
extern void gap_send_msg_to_app(T_IO_MSG * p_msg);
...
{
	//mesh_node.node_state represents whether the device has been added to the mesh network
	//PROV_NODE means means YES. UNPROV_DEVICE means NO.
	//the state of provisioner is PROV_NODE forcibly
	data_uart_debug("MeshState:\t%d\r\n", mesh_node.node_state);

	//mesh_node.dev_uuid represents the uuid of devie.
	...
	//every device key represents one device in the mesh network.
	//every device has one element at least. so .element_num is unexpected.  
	...
	//every app key has global index and local index. 
	//.app_key_index_g is global index. and index of app_key_list is local index.
	//.key_state represents the state of Key Refresh procedure. MESH_KEY_STATE_INVALID means key is INVALID
	//.papp_key store old and new app key in Key Refresh procedure.
	// key_state_to_tx_loop func can find the index of current used appkey for tx in .papp_key, according to .key_state.
	// every app key need bind to one netKey. .net_key_binding is the local index of netkey that appkey bind.
	...
	//every net key has global index and local index.
	//net_key_index_g is global index. and index of net_key_list is local index
	//.key_state represents the state of Key Refresh procedure. MESH_KEY_STATE_INVALID means key is INVALID
	//.pnet_key store old and new net key in Key Refresh procedure.
	// key_state_to_tx_loop func can find the index of current used netkey for tx in .pnet_key, according to .key_state.
	...
	//mesh_node.element_queue.count represents the element numbers of mesh node.
...
user_cmd_parse_result_t user_cmd_node_deinit(user_cmd_parse_value_t *pparse_value)
{
    mesh_node_deinit();
    return USER_CMD_RESULT_OK;
}
