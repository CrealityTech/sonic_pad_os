...
static int getInfoFromCmdline(char *key, char *value)
{
    ...
    if (!fd) {
        SST_DBG_ERR("Failed to open %s", CMDLINE_FILE_PATH);
        return -1;
    }

    ret = read(fd, cmdline, 1024);
    if (ret < 0 || ret > 1024) {
        SST_DBG_ERR("Cmd line read fail: ret=%d, %s\n", ret, strerror(errno));
        close(fd);
        return -1;
    }
    close(fd);
...
static int get_flash_type(void)
{
    ...
    if (getInfoFromCmdline("boot_type=", ctype)) {
        SST_DBG_ERR("Get boot type cmd line fail\n");
        return -1;
    }

    flash_boot_type = atol(ctype);
    SST_DBG_INFO("Boot type %d\n", flash_boot_type);
    return 0;
}
...
static int _nand_read(int id, unsigned char *buf, ssize_t len)
{
    ...
    if (fd < 0) {
        SST_DBG_ERR("Failed to open %s: %s", nand_oem_path, strerror(errno));
        return -1;
    }
...
static int nv_read(int id, unsigned char *buf, ssize_t len)
{
    ...
    case FLASH_TYPE_SD2:
        ...
        break;
    default:
        SST_DBG_ERR("Unknown no-volatile device\n");
        ret = -1;
        break;
    }
