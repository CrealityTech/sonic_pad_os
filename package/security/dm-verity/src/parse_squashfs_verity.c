...
int main(int argc, char *argv[])
{
	...
	fd_sign = open(argv[2], O_CREAT | O_RDWR | O_TRUNC, 0666);
	if (fd_sign == -1) {
		printf("Could not open sign file: %s - %s\n", argv[2], strerror(errno));
		goto out;
	}

	fd_root_hash = open(argv[3], O_CREAT | O_RDWR | O_TRUNC, 0666);
	if (fd_root_hash == -1) {
		printf("Could not open root_hash file: %s - %s\n", argv[3], strerror(errno));
		goto out;
	}

	fd_hash_tree = open(argv[4], O_CREAT | O_RDWR | O_TRUNC, 0666);
	if (fd_hash_tree == -1) {
