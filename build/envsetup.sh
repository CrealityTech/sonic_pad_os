...
function swupdate_pack_swu() {
	...
	local PYTHON_TOOL="create_c-smart-brain_install_config.py"
	mkdir -p "$SWU_DIR"
	if [ "x$1" = "x-ab-rdiff" ]; then
		local delta_dir="$SWU_DIR/swupdate_delta"
		local diff_version=$(cat $delta_dir/version)
	fi
	local storage_type_nor=0
	local f="$T/device/config/chips/${TARGET_PLATFORM}/configs/${TARGET_PLAN}/sys_config.fex"
	local B="$( awk -F"=" '/^storage_type/{print $2}' $f | sed 's/^[ \t]*//g' )"
	...
	if [ -e $SWUPDATE_CONFIG_DIR/$CFG ]; then
		...
		if [ -e $SWUPDATE_CONFIG_DIR/$PYTHON_TOOL ]; then
			local SWUPDATE_CREATE_CONFIG="$SWUPDATE_CONFIG_DIR/$PYTHON_TOOL"
		fi
	elif [ -e $SWUPDATE_COMMON_CONFIG_DIR/$CFG ]; then
		local SWUPDATE_SUBIMGS="$SWUPDATE_COMMON_CONFIG_DIR/$CFG"
		if [ -e $SWUPDATE_COMMON_CONFIG_DIR/$PYTHON_TOOL ]; then
			local SWUPDATE_CREATE_CONFIG="$SWUPDATE_COMMON_CONFIG_DIR/$PYTHON_TOOL"
		fi
	else
		local SWUPDATE_SUBIMGS="$SWUPDATE_GENERIC_CONFIG_DIR/$CFG"
		if [ -e $SWUPDATE_GENERIC_CONFIG_DIR/$PYTHON_TOOL ]; then
			local SWUPDATE_CREATE_CONFIG="$SWUPDATE_GENERIC_CONFIG_DIR/$PYTHON_TOOL"
		fi
	fi
	...
	echo "-------------------- create creality ota -------------------"
	if [ "x$1" = "x-ab-rdiff" ]; then
		local version=${diff_version}
	else
		local version=$(sed -n "3p" $SWU_DIR/sw-description | sed  's/\t//g; s/;//g; s/ //g; s/"//g' | awk -F = '{print $2}')
	fi

	echo "swu_version: $version"
	mv $SWU_DIR/tina-${TARGET_BOARD}$1.swu $SWU_DIR/t800-${TARGET_BOARD#r818-}$1_$version.swu

	rm -f "$SWU_DIR/config.ini"
	if [ -e $SWUPDATE_CREATE_CONFIG ]; then
		python3 $SWUPDATE_CREATE_CONFIG $SWU_DIR/t800-${TARGET_BOARD#r818-}$1_$version.swu $SWU_DIR/
	fi
	echo ""

	print_red "$SWU_DIR/t800-${TARGET_BOARD#r818-}$1_$version.swu"
	du -sh "$SWU_DIR/t800-${TARGET_BOARD#r818-}$1_$version.swu"
	echo ""

	local password=$(echo -n "t800-${TARGET_BOARD#r818-}$1_${version}crealitysonic" | md5sum | cut -d ' ' -f 1)
	7z a -p$password $SWU_DIR/t800-${TARGET_BOARD#r818-}$1_$version.7z $SWU_DIR/t800-${TARGET_BOARD#r818-}$1_$version.swu $SWU_DIR/config.ini
...
function swupdate_make_delta()
{
	...
	local base_swu_filename=$(basename $base_swu)
	local new_swu_filename=$(basename $new_swu)
	local base_version=${base_swu_filename##*_}
	local new_version=${new_swu_filename##*_}
	local base_new_version=${base_version%.swu}_${new_version%.swu}
	local delta_dir="$SWU_DIR/swupdate_delta"
	...
	echo "base_new version : $base_new_version"
	...
	echo "$base_new_version" > $delta_dir/version
