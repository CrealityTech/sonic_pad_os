...
static int sunxi_mmc_judge_retry_v4p5x(struct sunxi_mmc_host *host,
...
TO_HS:
		/* Reset and disabled mmc_avail_type to switch speed mode to HS */
		dev_info(mmc_dev(host->mmc), "sunxi v4p5x retry give up, return to HS\n");
		...
		dev_info(mmc_dev(host->mmc), "sunxi v4p5x retry give up!\n");
		return -1;
	}
}
			       ...
			       struct platform_device *pdev, int phy_index)
{
	...
	host->sunxi_mmc_judge_retry = sunxi_mmc_judge_retry_v4p5x;
