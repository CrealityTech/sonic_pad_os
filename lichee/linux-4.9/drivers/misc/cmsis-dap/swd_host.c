...
const  uint32_t Flash_Page_Size=1024;
...
uint8_t swd_read_memory(uint32_t address, uint8_t *data, uint32_t size)
{
    ...
    while (size > 3) {
        ...
        n = DOWNLOAD_BLOCK_SIZE - (address & (DOWNLOAD_BLOCK_SIZE - 1));
...
// #ifdef CUSTOM_LIAN
//     printk("%s: line %d custom_lian swd_write_memory \n", __func__, __LINE__);
// #endif
...
// #ifdef CUSTOM_LIAN
//     printk("%s: line %d custom_lian size1: %d， address: 0x%x\n", __func__, __LINE__, size);
// #endif
...
// #ifdef CUSTOM_LIAN
//     printk("%s: line %d custom_lian size2 %d\n", __func__, __LINE__, size);
// #endif
...
uint8_t JTAG2SWD(void)
{
    ...
    if (!swd_reset()) {
		printk("%s: line %d swd_reset() fail\n", __func__, __LINE__);
        return 0;
    }
    ...
    if (!swd_read_idcode(&tmp)) {
		printk("%s: line %d swd_read_idcode() return 0x%x\n", __func__, __LINE__, tmp);
        return 0;
    }
...
/*
	if(asserted == 0)
		swd_write_word((uint32_t)SCB_AIRCR_ADDR, ((0x5FA << SCB_AIRCR_VECTKEY_Pos) | (SCB_AIRCR_REGVAL & SCB_AIRCR_PRIGROUP_Msk) | SCB_AIRCR_SYSRESETREQ_Msk));
*/
...
#ifdef CUSTOM_LIAN
    printk("%s: line %d custom_lian swd_set_target_state_hw \n", __func__, __LINE__);
#endif
...
#ifdef CUSTOM_LIAN
    printk("%s: line %d custom_lian ap_retries %d\n", __func__, __LINE__, ap_retries);
#endif
            ...
            do {
                if (!swd_read_word(DBG_HCSR, &val)) {
                    printk("%s: line %d Swd read word fail\n", __func__, __LINE__);
                    return 0;
                }
                printk("%s: line %d val = 0x%x, 0x%x\n", __func__, __LINE__, val, S_HALT);

            } while ((val & S_HALT) == 0);

#ifdef CUSTOM_LIAN
    printk("%s: line %d custom_lian val %d\n", __func__, __LINE__, val);
#endif
