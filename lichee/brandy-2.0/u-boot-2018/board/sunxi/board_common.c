...
int sunxi_boot_sys_led(void)
{
#define FDT_PATH_BOOT_SYS_LED_GPIO "/leds/sys_led/"
	user_gpio_set_t gpio_init;
	int nodeoffset;
	if (get_boot_work_mode() != WORK_MODE_BOOT) {
		return 0;
	}
	nodeoffset = fdt_path_offset(working_fdt, FDT_PATH_BOOT_SYS_LED_GPIO);
	if (IS_ERR_VALUE(nodeoffset)) {
		return 0;
	}
	if (!fdtdec_get_is_enabled(working_fdt, nodeoffset)) {
	...
	if (fdt_get_one_gpio(FDT_PATH_BOOT_SYS_LED_GPIO, "gpios", &gpio_init) ==
			0) {
		sunxi_gpio_request(&gpio_init, 1);
	}
