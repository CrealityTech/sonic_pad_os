...
static int nonstandard_addr_parse(const char *addr_str, char *dst_addr)
{
	char tmp_addr[18]= {0};
	char *p = (char *)addr_str;
	...
	for (i = 0; i < 6; i++) {
		if(i < 5)
		{
			sprintf(tmp_addr, "%s%c%c:", tmp_addr, p[0], p[1]);
		}else{
			sprintf(tmp_addr, "%s%c%c", tmp_addr, p[0], p[1]);
		}
		p += 2;
	...
	for (i = 0; i < ARRAY_SIZE(addr); i++) {
		...
		if (p != NULL) {
			if (addr_parse(p, addr[i].flag)) {
				...
				if(nonstandard_addr_parse(p, tmp_addr) == 0) {
					pr_err("%s format illegal and reset addr: %s success\n", addr[i].envname, tmp_addr);
					env_set(addr[i].envname, tmp_addr);
				} else {
