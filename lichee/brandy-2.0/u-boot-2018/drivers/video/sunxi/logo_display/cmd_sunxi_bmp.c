...
int sunxi_bmp_display(char *name)
{
	...
	logo_rotate = env_get("logo_rotate");
	if (NULL != logo_rotate)
		tick_printf("logo_rotate=%s\n", logo_rotate);

	partno = sunxi_partition_get_partno_byname("bootloader"); /*android*/
	if (partno < 0) {

		if((NULL != logo_rotate) && !strcmp(logo_rotate, "1")) {
			partno = sunxi_partition_get_partno_byname(
			    "boot-resource1"); /*linux*/
			if (partno < 0) {
				pr_error("Get bootloader and boot-resource1 partition number fail!\n");
				return -1;
			}
		} else {
			partno = sunxi_partition_get_partno_byname(
