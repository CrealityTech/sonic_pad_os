...
static u16 geth_phy_read(struct eth_device *dev, int phy_adr, u16 reg)
{
	...
	writel(reg_val, (u32)(dev->iobase + GETH_MDIO_ADDR));
	while (readl((u32)(dev->iobase + GETH_MDIO_ADDR)) & MII_BUSY)
		;

	return  readl((u32)(dev->iobase + GETH_MDIO_DATA));
}
...
static void geth_phy_write(struct eth_device *dev, u8 phy_adr, u8 reg, u16 data)
{
	...
	writel(data, (u32)(dev->iobase + GETH_MDIO_DATA));
	writel(reg_val, (u32)(dev->iobase + GETH_MDIO_ADDR));
...
static int geth_xmit(struct eth_device *dev, void *packet, int length)
{
	...
	/* Get transmit status */
	xmit_stat = readl((u32)(dev->iobase + GETH_TX_DMA_STA)) & 0x7;
	...
	/* flush Transmit FIFO */
	reg_val = readl((u32)(dev->iobase + GETH_TX_CTL1));
	reg_val |= 0x00000001;
	writel(reg_val, (u32)(dev->iobase + GETH_TX_CTL1));
...
static int geth_recv(struct eth_device *dev)
{
	...
	recv_stat = readl((u32)(dev->iobase + GETH_INT_STA));
	if (!(recv_stat & 0x2300))
		goto fill;

	writel(0x3F00, (u32)(dev->iobase + GETH_INT_STA));
...
fill:
	...
	recv_stat = readl((u32)(dev->iobase + GETH_RX_DMA_STA)) & 0x07;
	/* Enable receive and poll it */
	reg_val = readl((u32)(dev->iobase + GETH_RX_CTL1));
	if (recv_stat == 0x00)
		reg_val |= 0x40000000;
	else
		reg_val |= 0x80000000;
	writel(reg_val, (u32)(dev->iobase + GETH_RX_CTL1));
...
/* it should be defined in uboot/include/configs/sun?iw?p?.h */
	...
	/* Enable clk for ephy */
	value = readl((u32)PHY_CLK_REG);
	if (used_type == INT_PHY) {
		reg_val = readl((u32)(CCMU_BASE + 0x0070));
		reg_val |= (1 << 0);
		writel(reg_val, (u32)(CCMU_BASE + 0x0070));

		reg_val = readl((u32)(CCMU_BASE + 0x02c8));
		reg_val |= (1 << 2);
		writel(reg_val, (u32)(CCMU_BASE + 0x02c8));
	...
	writel(value, (u32)PHY_CLK_REG);

#if defined(CONFIG_SUN50IW6P1) || defined(CONFIG_SUN8IW12P1) \
    || defined(CONFIG_SUN8IW12P1_NOR) || defined(CONFIG_MACH_SUN50IW9)
	/* enalbe clk for gmac */
	reg_val = readl((u32)(CCMU_BASE + CCMU_GMAC_CLK_REG));
	reg_val |= (1 << CCMU_GMAC_RST_BIT);
	reg_val |= (1 << CCMU_GMAC_GATING_BIT);
	writel(reg_val, (u32)(CCMU_BASE + CCMU_GMAC_CLK_REG));
#else
	/* enalbe clk for gmac */
	reg_val = readl((u32)(CCMU_BASE + AHB1_GATING));
	reg_val |= AHB1_GATING_BIT;
	writel(reg_val, (u32)(CCMU_BASE + AHB1_GATING));

	reg_val = readl((u32)(CCMU_BASE + AHB1_RESET));
	reg_val |= AHB1_RESET_BIT;
	writel(reg_val, (u32)(CCMU_BASE + AHB1_RESET));
#endif

	/* enable ephy clk */
	if (use_ephy_clk == 1) {
		printf("gmac: *** using ephy_clk ***\n");
		reg_val = readl((u32)(CCMU_BASE + CCMU_EPHY_CLK_REG));
...
#endif
		writel(reg_val, (u32)(CCMU_BASE + CCMU_EPHY_CLK_REG));
	}
...
static int mii_phy_init(struct eth_device *dev)
{
	...
	phy_val = geth_phy_read(dev, phy_addr, MII_BMCR);
	reg_val = readl((u32)(dev->iobase + GETH_BASIC_CTL0));
	...
	if (phy_val & BMCR_SPEED100)
		...
		reg_val |= 0x08;
	writel(reg_val, (u32)(dev->iobase + GETH_BASIC_CTL0));
...
static int geth_init(struct eth_device *dev, bd_t *bis)
{
	...
	writel(0x01, (u32)(dev->iobase + GETH_BASIC_CTL1));
	while (readl((u32)(dev->iobase + GETH_BASIC_CTL1)) & (0x01))
		;
	...
	/* Initialize core */
	reg_val = readl((u32)(dev->iobase + GETH_TX_CTL0));
	reg_val |= (3 << 30);   /* Enable transmit component &  Jabber Disable */
	writel(reg_val, (u32)(dev->iobase + GETH_TX_CTL0));
	...
	/* GMAC frame filter */
	reg_val = readl((u32)(dev->iobase + GETH_RX_FRM_FLT));
	reg_val |= 0x00000001;
	writel(reg_val, (u32)(dev->iobase + GETH_RX_FRM_FLT));

	/* Burst should be 8 */
	reg_val = readl((u32)(dev->iobase + GETH_BASIC_CTL1));
	reg_val |= (8 << 24);
	writel(reg_val, (u32)(dev->iobase + GETH_BASIC_CTL1));
	...
	writel(0x00UL, (u32)(dev->iobase + GETH_INT_EN));
	...
	writel((u32)dma_desc_tx, (u32)(dev->iobase + GETH_TX_DESC_LIST));
	writel((u32)dma_desc_rx, (u32)(dev->iobase + GETH_RX_DESC_LIST));
...
int geth_write_hwaddr(struct eth_device *dev)
{
	...
	addr = &(dev->enetaddr[0]);
	mac  = (addr[5] << 8) | addr[4];
	writel(mac, (u32)(dev->iobase + GETH_ADDR_HI(0)));

	mac  = (addr[3] << 24) | (addr[2] << 16) | (addr[1] << 8) | addr[0];
	writel(mac, (u32)(dev->iobase + GETH_ADDR_LO(0)));
...
int geth_initialize(bd_t *bis)
{
	...
	dma_desc_tx = (struct dma_desc *)buf_addr;
    if (dma_desc_tx == NULL)
		goto err;

	buf_addr = (u32)noncached_alloc(sizeof(struct dma_desc), 64);
	dma_desc_rx = (struct dma_desc *)buf_addr;
	if (dma_desc_rx == NULL)
		goto err;

	buf_addr = (u32)noncached_alloc(2048, 64);
	rx_packet = (char *)buf_addr;
	if (rx_packet == NULL)
		goto err;

	buf_addr = (u32)noncached_alloc(2048, 64);
	rx_handle_buf = (char *)buf_addr;
