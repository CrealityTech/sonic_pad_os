 ...
 */
s32 getcmd(void *parg __attribute__ ((__unused__)),
...
static int do_set_baudrate(const char **args)
{
	...
	uart_set_baudrate(&debug_uart,baud);
 ...
 * @args: parameters
 */
static int do_help(const char **args __attribute__ ((__unused__)))
{
	const struct builtincmd *bp;
