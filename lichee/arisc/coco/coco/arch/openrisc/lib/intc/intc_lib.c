...
{
	*buf++ = pintc_regs->enable;
	*buf++ = pintc_regs->enable1;
	*buf = pintc_regs->enable2;
}

void Intc_Save_Mask(u32 *buf)
{
	*buf++ = pintc_regs->mask;
	*buf++ = pintc_regs->mask1;
	*buf = pintc_regs->mask2;
}

void Intc_Save_Pending(u32 *buf)
{
	pintc_regs->pending = *buf++;
	pintc_regs->pending1 = *buf++;
	pintc_regs->pending2 = *buf;
}


void Intc_Restore_Enable(u32 *buf)
{
	pintc_regs->enable = *buf++;
	pintc_regs->enable1 = *buf++;
	pintc_regs->enable2 = *buf;
}

void Intc_Restore_Mask(u32 *buf)
{
	pintc_regs->mask = *buf++;
	pintc_regs->mask1 = *buf++;
	pintc_regs->mask2 = *buf;
}

void Intc_Restore_Pending(u32 *buf)
{
	*buf++ = pintc_regs->pending;
	*buf++ = pintc_regs->pending1;
	*buf = pintc_regs->pending2;
}
/*
*********************************************************************************************************
*                                         INTERRUPT EXIT
