...
void UART_Config(UART_TypedDef *UARTx, UART_InitConfig *config)
{
	...
	UARTx=UARTx;
	writel(0x1,0x0701018c);
	writel(0x10001,0x0701018c);
	apbs_clk = 24000000;
	divisor = (apbs_clk + (config->UART_BaudRate << 3))/(config->UART_BaudRate << 4);
	writel(0x7,0x07080008);
	writel(0x1,0x070800a4);
	writel(0x80,0x0708000c);
	writel(divisor >> 8 ,0x07080004);
	writel(divisor & 0xff ,0x07080000);
	writel(readl(0x0708000c) & (~0x80),0x0708000c);
	writel(0x0,0x070800a4);
	writel(0x3,0x0708000c);
	writel(0x7,0x07080008);
